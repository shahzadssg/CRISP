...
def randomise_lsb_plane(image: np.ndarray, channel: int) -> np.ndarray:
    """
    Repair R1.  Overwrite the whole LSB plane of `channel` with fresh
    uniform bits.

    Without this, the decoy bits at every pixel other than (row, col)
    are the cover's natural LSBs.  Natural LSB planes are spatially
    correlated and biased, while the embedded bit is uniform and
    independent, and that mismatch is exactly the signal RS and
    sample-pair steganalysis exploit.  R1 turns the i.i.d. uniform
    decoy model from an assumption about photographs into a property
    of the scheme.

    Costs one pass over the image and nothing in communication.
    Correctness is unaffected: Alice writes the secret bit afterwards.
    """
    out   = image.copy()
...
# ===========================================================================
# Repair R2: constant wires as pixel-uniform planes, generated by Carol
# ===========================================================================
def carol_materialise_constants(circuit:     Circuit,
                                image_shape: Tuple[int, int, int]
                                ) -> Dict[str, Tuple[np.ndarray, int]]:
    """
    Repair R2.  Build one plane per public-constant ancillary, carrying
    the constant in the LSB of channel 0 at EVERY pixel.

    Carol generates these locally from the circuit specification, so
    Alice transmits no carrier for them.  That is the whole point.

    Embedding a known constant only at (row, col), the way
    alice_prepare_sources used to, lets Carol discard every pixel whose
    LSB disagrees with the value she already knows.  Each such wire
    halves the candidate pixel set, so kappa constants take the joint
    recovery bound from 1/n to roughly 2^kappa/(2^kappa + n - 1).
    See attack_constant_wires() for that leak measured end to end.

    A pixel-uniform plane carries no positional information because
    every pixel satisfies the constraint.  The price is that its LSB
    channel is degenerate and therefore not undetectable, which is
...
# ===========================================================================
# Alice: prepare source images for primary inputs
# ===========================================================================
def alice_prepare_sources(circuit:     Circuit,
    ...
    """
    Alice creates one stego image per primary-input wire, embedding the
    bit at (row, col) in channel 0 after re-randomising that channel's
    LSB plane (repair R1).

    Public-constant ancillaries get no carrier at all (repair R2).
    Carol builds those herself via carol_materialise_constants(), which
    both removes |W_const| images from the transmission and removes the
    positional leak they used to cause.

    Set apply_r1 or apply_r2 to False only to reproduce the pre-repair
    behaviour for measurement.

    Returns a wire-to-(image, channel) map for the transmitted sources.
    """
    ...
    if not apply_r2:
        # Pre-repair behaviour, kept only so the leak can be measured.
        # Each constant is embedded at (row, col) alone, in a carrier
        # Alice transmits.  attack_constant_wires() exploits exactly this.
        for wire, value in circuit.ancillaries:
            cover = source_pool[pool_idx]
            pool_idx += 1
            if apply_r1:
...
def run_crisp(circuit_spec:  str,
    ...
    # R2 removes the constant carriers from the transmitted set.
    ...
    transmitted = dict(wire_sources)   # what actually goes on the wire

    # ---- Carol builds the constant planes herself (R2) ----
    if apply_r2:
    ...
    timing = {
        ...
        "n_gates": n_gates,
        "images_transmitted": n_sources + 2 * n_gates,
        "constant_planes_local": n_const if apply_r2 else 0,
    }
    if return_view:
        view = {
...
# ===========================================================================
# Attack: constant-wire candidate filtering  (paper: Corollary 1)
# ===========================================================================
def attack_constant_wires(view: Dict) -> Dict:
    """
    Carol's view only.  She holds the circuit specification, so she knows
    which wires are ancillaries and what constant each one carries.  For
    every transmitted constant carrier she discards each pixel whose LSB
    disagrees with the value she already knows.

    The secret pixel always survives, so the survivors are her candidate
    set and her optimal strategy is to guess uniformly among them.  She
    wins with probability 1/|candidates|.

    Under repair R2 no constant carrier is transmitted, nothing gets
    filtered, and the candidate set is the whole image.
    """
    circuit = view["circuit"]
    h, w, _ = view["image_shape"]
    row, col = view["secret"]
    sources = view["transmitted_sources"]

    candidates = np.ones((h, w), dtype=bool)
    used = 0
    for wire, value in circuit.ancillaries:
        if wire not in sources:          # R2: never transmitted
            continue
        img, ch = sources[wire]
        candidates &= ((img[:, :, ch] & 1) == value)
        used += 1
    ...
    return {
        "kappa": used,
        "candidates": n_cand,
        "secret_survives": bool(candidates[row, col]),
        "adv": 1.0 / n_cand,
        "adv_if_uniform": 1.0 / (h * w),
        "degradation": (h * w) / n_cand,
        "pinned": n_cand == 1,
    }


def demo_repairs(image_shape=(128, 128, 3), trials=200, seed=7):
    """Measure the leak with R2 off, then with R2 on."""
    random.seed(seed); np.random.seed(seed)
    n = image_shape[0] * image_shape[1]
    print(f"Constant-wire attack on the benchmark circuit, "
          f"{image_shape[0]}x{image_shape[1]} covers, {trials} trials\n")
    print(f"{'setting':<12} {'kappa':>6} {'imgs sent':>10} {'mean cands':>11} "
          f"{'measured adv':>13} {'1/n':>11} {'worse by':>9}")
    print("-" * 78)
    for label, r2 in [("R2 off", False), ("R2 on", True)]:
        cands, advs, imgs, kap, ok = [], [], [], 0, True
        for _ in range(trials):
            out, timing, view = run_crisp(
                BENCHMARK_CIRCUIT, {"A": 1, "B": 0, "C": 1},
                image_shape=image_shape, apply_r2=r2, return_view=True)
            a = attack_constant_wires(view)
            ok &= a["secret_survives"] and out["result"] == 1
            cands.append(a["candidates"]); advs.append(a["adv"])
            imgs.append(timing["images_transmitted"]); kap = a["kappa"]
        mc, ma = sum(cands) / trials, sum(advs) / trials
        print(f"{label:<12} {kap:>6} {imgs[0]:>10} {mc:>11.1f} "
              f"{ma:>13.3e} {1/n:>11.3e} {ma*n:>8.1f}x")
        assert ok, "correctness or survival broke"
    print("\nCorrectness held in every trial and the secret pixel always survived.")


def known_constant_wires(circuit: Circuit) -> Dict[str, int]:
    """
    Every wire whose value Carol can derive from the public circuit
    specification alone, by constant propagation from the ancillaries.

    This is what makes obfuscation actively harmful.  The dummy gates it
    inserts take all three inputs from dummy ancillaries, so a Fredkin
    gate on three known constants has three known constant outputs, and
    each one is a further filter on the candidate pixel set.
    """
...
def attack_full_transcript(view: Dict) -> Dict:
    """
    The complete attack.  Carol filters on every wire whose value the
    public specification determines: the constant ancillaries and every
    wire reachable from them by constant propagation.  She uses the gate
    output images she produced herself, not just Alice's carriers.
    """
    circuit = view["circuit"]
    h, w, _ = view["image_shape"]
    row, col = view["secret"]
    ...
    candidates = np.ones((h, w), dtype=bool)
    used = 0
    for wire, value in known_constant_wires(circuit).items():
        if wire not in wires:
            continue
        img, ch = wires[wire]
        plane = (img[:, :, ch] & 1) == value
        if plane.all():
            continue          # pixel-uniform under R2: filters nothing
        candidates &= plane
        used += 1

    n_cand = int(candidates.sum())
    return {"kappa_effective": used, "candidates": n_cand,
            "secret_survives": bool(candidates[row, col]),
