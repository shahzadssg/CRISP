...
```
If you wish to run the code with interactive image pop-ups (not recommended for server/headless environments), you might need `tkinter` for `matplotlib`'s `TkAgg` backend:
```bash
...
---
**Reference:**
Ahmad, S., & Rass, S. (2024). *ProSt: Provably Secure Homomorphic Steganography for Circuit Evaluation*. [Placeholder for actual publication details].
```